...
def main() -> None:
    ...
    mock_csv = mock_df.to_csv(index=False, encoding="utf-8-sig").encode("utf-8-sig")
    st.download_button(
    ...
    if uploaded_file is not None:
        try:
            ...
            missing_columns = validate_required_columns(raw_uploaded_df)
            if missing_columns:
                upload_error = (
                    "파일 업로드 시 필수 컬럼(JE_NUM, POSTING_DATE, INPUT_TIME, "
                    "ACCOUNT_NAME, DEBIT, CREDIT, DESCRIPTION)이 포함되어 있어야 합니다. "
                    f"누락 컬럼: {', '.join(missing_columns)}"
                )
            else:
                journal_df = prepare_journal_dataframe(raw_uploaded_df)
